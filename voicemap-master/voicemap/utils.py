...
def get_bottleneck(classifier, samples):
    ...
    # Get activations
    layer_outs = functor([samples, 0.0])
...
def whiten(batch, rms=0.038021):
    ...
    if len(batch.shape) != 3:
        raise (
            ValueError,
            "Input must be a 3D array of shape (n_segments, n_timesteps, 1).",
        )

    # Subtract mean
    sample_wise_mean = batch.mean(axis=1)
    whitened_batch = batch - np.tile(
        sample_wise_mean, (1, 1, batch.shape[1])
    ).transpose((1, 2, 0))

    # Divide through
    sample_wise_rescaling = rms / np.sqrt(np.power(batch, 2).mean())
    whitened_batch = whitened_batch * np.tile(
        sample_wise_rescaling, (1, 1, batch.shape[1])
    ).transpose((1, 2, 0))

    return whitened_batch


def n_shot_task_evaluation(
    model,
    dataset,
    preprocessor,
    num_tasks,
    ...
    if n == 1 and network_type == "siamese":
        ...
        for i_eval in tqdm((range(num_tasks))):
            ...
            support_set_instances = preprocessor.instance_preprocessor(
            ...
            if distance == "euclidean":
                ...
                for i in range(0, n * k, n):
                    mean_support_set_embeddings.append(
                        support_set_embeddings[i : i + n, :].mean(axis=0)
                    )
                ...
                pred = np.sqrt(
                    np.power(
                        (
                            np.concatenate([query_embedding] * k)
                            - mean_support_set_embeddings
                        ),
                        2,
                    ).sum(axis=1)
                ...
                magnitudes = np.linalg.norm(
                    support_set_embeddings, axis=1, keepdims=True
                )
                unit_vectors = support_set_embeddings / magnitudes
                mean_support_set_unit_vectors = []
                for i in range(0, n * k, n):
                    mean_support_set_unit_vectors.append(
                        unit_vectors[i : i + n, :].mean(axis=0)
                    )
                ...
                mean_support_set_embeddings = (
                    mean_support_set_magnitudes * mean_support_set_unit_vectors
                )

                # Get dot product between mean embeddings
                pred = np.dot(
                    query_embedding[0, :][np.newaxis, :], mean_support_set_embeddings.T
                )
...
class NShotEvaluationCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        n_shot_acc = n_correct * 1.0 / self.num_tasks
