...
class LibriSpeechDataset(Sequence):
    ...
    def __init__(
        ...
        self.df = self.df.rename(
            columns={"id": "speaker_id", "minutes": "speaker_minutes"}
        )
    ...
    def __getitem__(self, index):
        ...
        if self.label == "sex":
            ...
            raise (
                ValueError,
                "Label type must be one of ('sex', 'speaker')".format(self.label),
            )
    ...
    def get_alike_pairs(self, num_pairs):
        ...
        alike_pairs = pd.merge(
            self.df.sample(num_pairs * 2, weights="length"), self.df, on="speaker_id"
    ...
    def get_differing_pairs(self, num_pairs):
        ...
        random_sample_from_other_speakers = self.df[
            ~self.df["speaker_id"].isin(random_sample["speaker_id"])
        ].sample(num_pairs, weights="length")

        differing_pairs = list(
            zip(
                random_sample["id"].values,
                random_sample_from_other_speakers["id"].values,
            )
        )
    ...
    def build_verification_batch(self, batchsize):
        ...
        input_1_different = np.stack(
            [self[i][0] for i in list(zip(*differing_pairs))[0]]
        )
        input_2_different = np.stack(
            [self[i][0] for i in list(zip(*differing_pairs))[1]]
        )

        input_1 = np.vstack([input_1_alike, input_1_different])[:, :, np.newaxis]
        input_2 = np.vstack([input_2_alike, input_2_different])[:, :, np.newaxis]

        outputs = np.append(np.zeros(batchsize // 2), np.ones(batchsize // 2))[
            :, np.newaxis
        ]
    ...
    def build_n_shot_task(self, k, n=1):
        ...
        if k >= self.unique_speakers:
            raise (
                ValueError,
                "k must be smaller than the number of unique speakers in this dataset!",
            )
        ...
        correct_samples = self.df[is_query_speaker & not_same_sample].sample(
            n, weights="length"
        )
        ...
        support_set_samples = tuple(
            np.stack(i) for i in zip(*[self[i] for i in support_set.index])
        )
    ...
    def index_subset(subset):
        ...
        for root, folders, files in os.walk(
            ...
            for f in files:
                ...
                if not f.endswith(".flac") and not f.endswith(".wav"):  ##Add wav files
                    continue
                ...
                audio_files.append(
                    {
                        ...
                        "length": len(instance),
                        "seconds": len(instance) * 1.0 / LIBRISPEECH_SAMPLING_RATE,
                    }
                )
