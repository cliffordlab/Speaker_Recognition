...
class TestLibriSpeechDataset(unittest.TestCase):
    ...
    def test_n_shot_task(self):
        ...
        for i in range(0, n * k, n):
            support_set_classes_correct = np.all(
                support_set_labels[i : i + n] == support_set_labels[i]
            )
            self.assertTrue(
...
class TestWhitening(unittest.TestCase):
    def test_whitening_no_batch(self):
        ...
        test_data, sample_rate = sf.read(
            PATH + "/data/LibriSpeech/dev-clean/84/121123/84-121123-0000.flac"
        )
        ...
        # Mean correct
        self.assertTrue(
            np.isclose(whitened.mean().item(), 0), "Whitening should reduce mean to 0."
        )
