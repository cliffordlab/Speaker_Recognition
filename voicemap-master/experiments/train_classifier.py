...
pad = True
num_epochs = 50
evaluate_every_n_batches = 10  # 500
num_evaluation_tasks = 10  # 500
n_shot_classification = 1
k_way_classification = 5

# Derived parameters
input_length = int(LIBRISPEECH_SAMPLING_RATE * n_seconds / downsampling)
param_str = "classifier__filters_{}__embed_{}__drop_{}__pad={}".format(
...
class BatchedSequence(Sequence):
    ...
    def __init__(self, sequence, batch_preprocessor, batchsize):
        ...
        np.random.shuffle(list(self.underlying_indexes))
        self.batch_to_index = {
...
# Define model #
################
classifier = get_baseline_convolutional_encoder(
    filters, embedding_dimension, (input_length, 1)
)
...
classifier.fit_generator(
    ...
    callbacks=[
        ...
        ModelCheckpoint(
            PATH + "/models_new/{}.hdf5".format(param_str),
